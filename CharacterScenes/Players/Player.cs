...
public partial class Player : CharacterBody2D
{
    ...
    {
        GetNode<HealthComponent>("IgnoreCamera/HealthComponent").Heal(healAmount);
    }
    public override void _Ready()
    {
        ...
        GetNode<Label>("IgnoreCamera/Coin/Label").Text = " = " + Coins;
        ...
        if ((bool)Config.GetValue("Player", "HasWeapon", false))
        {
            if (IsInGroup((string)Config.GetValue("Player", "WeaponClass", "null")))
            {
                attackDmg += (int)Config.GetValue("Player", "WeaponAtk", 0);
                Speed += (int)Config.GetValue("Player", "WeaponSpeed", 0);
                GetNode<HealthComponent>("IgnoreCamera/HealthComponent").IncreaseMax((int)Config.GetValue("Player", "WeaponHealth", 0));
            }
        }
    }
    public void Save()
